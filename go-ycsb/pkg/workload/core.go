...
func (c *core) GenerateTransaction(ctx context.Context, db ycsb.DB) (int64, string, string, []string, map[string][]byte) {
	state := ctx.Value(stateKey).(*coreState)
	r := state.r
	...
	switch operation {
	case read:
		return c.GenerateRead(ctx, db)
	case update:
		return c.GenerateUpdate(ctx, db)
	case insert:
		return c.GenerateInsert(ctx, db)
	default:
		return c.GenerateInsert(ctx, db)
	}
}

func (c *core) GenerateRead(ctx context.Context, db ycsb.DB) (int64, string, string, []string, map[string][]byte) {
	state := ctx.Value(stateKey).(*coreState)
	r := state.r
	keyNum := c.nextKeyNum(state)
	keyName := c.buildKeyName(keyNum)
	...
	return 1, c.table, keyName, fields, nil
}

func (c *core) GenerateInsert(ctx context.Context, db ycsb.DB) (int64, string, string, []string, map[string][]byte) {
	state := ctx.Value(stateKey).(*coreState)
	r := state.r
	keyNum := c.transactionInsertKeySequence.Next(r)
	dbKey := c.buildKeyName(keyNum)
	...
	return 3, c.table, dbKey, nil, values
}

func (c *core) GenerateUpdate(ctx context.Context, db ycsb.DB) (int64, string, string, []string, map[string][]byte) {
	state := ctx.Value(stateKey).(*coreState)
	keyNum := c.nextKeyNum(state)
	keyName := c.buildKeyName(keyNum)
	...
	return 2, c.table, keyName, nil, values
}

func (c *core) Execute(ctx context.Context, db ycsb.DB, op int64, table string, keyName string, fields []string, values map[string][]byte) error {
	var err error

	switch operationType(op) {
	case read:
		_, err = db.Read(ctx, table, keyName, fields)
		return err
	case update:
		return db.Update(ctx, table, keyName, values)
	case insert:
		return db.Insert(ctx, table, keyName, values)
	default:
		return nil
	}
}
