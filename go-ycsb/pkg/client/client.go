...
func (c *Client) Generate(ctx context.Context) (int64, string, string, []string, map[string][]byte) {
	w := newWorker(c.p, 0, 1, c.workload, c.db)
	wctx := c.workload.InitThread(ctx, 0, 1)
	//wctx = c.db.InitThread(wctx, 0, 1)
	...
	if w.doTransactions {
		op, table, keyName, fields, values = w.workload.GenerateTransaction(wctx, w.workDB)
	} else {
		op, table, keyName, fields, values = w.workload.GenerateInsert(wctx, w.workDB)
	}

	//c.db.CleanupThread(wctx)
	c.workload.CleanupThread(wctx)

	return op, table, keyName, fields, values
}

func (c *Client) Execute(ctx context.Context, op int64, table string, keyName string, fields []string, values map[string][]byte) {
	w := newWorker(c.p, 0, 1, c.workload, c.db)
	wctx := c.workload.InitThread(ctx, 0, 1)
	wctx = c.db.InitThread(wctx, 0, 1)

	w.workload.Execute(wctx, w.workDB, op, table, keyName, fields, values)

	c.db.CleanupThread(wctx)
	c.workload.CleanupThread(wctx)
}

func (c *Client) BatchExecuteCmd(ctx context.Context, trans []common.Transaction) {
	w := newWorker(c.p, 0, 1, c.workload, c.db)
	wctx := c.workload.InitThread(ctx, 0, 1)
	wctx = c.db.InitThread(wctx, 0, 1)
	...
	for i := 0; i < len(trans); i++ {
		tran := trans[i]
		op, table, keyName, fields, values := tran.Op, tran.Table, tran.KeyName, tran.Fields, tran.Values
		w.workload.Execute(wctx, w.workDB, op, table, keyName, fields, values)
	}
