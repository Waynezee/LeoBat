...
This branch is used to test Leobat-coin under [YCSB Workload](https://github.com/brianfrankcooper/YCSB) (we use [go-ycsb](https://github.com/pingcap/go-ycsb) in fact). Compared to basic LeoBat-coin, clients will use key-value store (Redis in memory) r/w commands generated by YCSB Workload instead of unmeaningful bytes. After commands are ordered by LeoBat, a thread will execute these commands  sequentially and then reply to the corresponding client.
## local test
